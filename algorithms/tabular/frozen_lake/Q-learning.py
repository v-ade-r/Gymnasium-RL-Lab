...
from repo_paths import FROZEN_LAKE_Q_LEARNING, FROZEN_LAKE_RESULTS
...
Q_VALUES_PATH = FROZEN_LAKE_Q_LEARNING
...
class FrozenLakeAgent():
    ...
    def get_action(self, obs: int, training: bool = True):
        prob = np.random.random()

        if training and prob < self.epsilon:
            return self.env.action_space.sample()   # Important to sample from the env, since that's the documented way!!!!
    ...
    def update(self, obs, action, reward, terminated, next_obs):
        future_q = (not terminated) * np.max(self.q_values[next_obs]) # IMPORTANT, a plain max, because we simply pick the value,
        # we don't care which action produces it.

        # np.max — you take the best possible action from the next state, regardless of what the agent would actually
        # do in that state. And what does the agent actually do? In get_action() it draws a random action with probability epsilon.
        #  So it collects data with one policy (epsilon-greedy), but learns as if it always played optimally (max).
        # This is the definition of off-policy — the policy collecting the data (behaviour policy) differs from the policy you
        # learn (target policy).
...
def plot_q_table_grid(agent, filename: str = 'Q_grid_Q_learning.png'):
    """
    ...
    ax.set_aspect('equal')
    ax.axis('off')
    ax.set_title('Frozen Lake 4x4 - Q values (Q-learning)\nLeft=Left | Down=Down | Right=Right | Up=Up',
                 fontsize=12)
...
def plot_results(rewards, steps):
    """
    Plots the rolling average of rewards to show learning progress.
    """
    ...
    path = RESULTS_DIR / "learning_curve_Q-learning.png"
    plt.savefig(path)
...
if __name__ == "__main__":
    ...
    if not smoke_mode():
        ...
        print_q_table(agent)
        plot_q_table_grid(agent, RESULTS_DIR / "Q_grid_Q_learning.png")
    save_q_values(agent)
